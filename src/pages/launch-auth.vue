...
const statusLabel = ref('Preparing secure publisher access...')
...
const nextPath = computed(() => {
  ...
  if (
    value === '/launch-auth' ||
    value.startsWith('/launch-auth?') ||
    value === '/managed-auth/callback' ||
    value.startsWith('/managed-auth/')
  ) {
    return '/dashboard'
  }
  return value
...
const handleLaunchBootstrap = async () => {
  ...
  statusLabel.value = 'Establishing publisher session...'
  await auth.init()

  const session = await waitForSession()
  if (!session) {
    throw new Error('The publisher session could not be established. Please launch the portal again from BPO Management.')
  }

  await auth.refreshProfile()

  statusLabel.value = 'Opening publisher dashboard...'
...
onMounted(async () => {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unable to complete the publisher launch.'
      ...
      <main class="mx-auto flex min-h-screen w-full max-w-3xl items-center justify-center px-6 py-10 lg:px-10">
        <section class="w-full rounded-4xl border border-white/10 bg-white/10 p-8 shadow-2xl shadow-black/50 backdrop-blur-xl">
          <div class="space-y-6 text-center">
            <div class="space-y-3">
              <p class="text-xs uppercase tracking-[0.4em] text-white/60">Publisher Portal Launch</p>
              <h1 class="text-3xl font-semibold text-white">Opening the publisher portal</h1>
              <p class="text-sm leading-6 text-white/70">
                We are securely starting this publisher session in a dedicated browser window.
              </p>
            </div>
            ...
            >
              <p class="text-sm font-semibold text-red-100">Publisher launch could not be completed</p>
